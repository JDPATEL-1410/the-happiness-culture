...
const Footer = () => {
  ...
  const certifications = [
    { label: 'SEBI Registered', icon: 'S' },
    { label: 'ISO Certified', icon: 'ISO' },
    { label: 'AMFI Member', icon: 'A' },
    { label: 'ARN-119542', icon: '✓' }
  ];
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          >
            ...
            <p className="text-gray-300 mb-6 leading-relaxed mx-auto md:mx-0 max-w-xs">
...
<motion.div
  ...
  className="text-center md:text-left"
>
  <h3 className="text-lg font-semibold mb-6">Try Our Calculators</h3>
  <ul className="space-y-3">
    {[
      ...
      <li
        key={index}
        className="footer-link flex items-center justify-center md:justify-start group"
      >
        <Link
          to={calc.path}
          className="flex items-center hover:text-success-400 transition-colors duration-300"
        >
          <span className="w-1.5 h-1.5 bg-success-500 rounded-full mr-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
          {calc.name}
        </Link>
  ...
  <div className="mt-5">
    <Link
      to="/calculator"
      className="inline-flex items-center justify-center md:justify-start text-success-400 hover:text-success-300 font-medium transition-colors duration-300 group"
    >
      ...
      <span className="ml-2 transform group-hover:translate-x-1 transition-transform duration-300">
            ...
            <div className="space-y-5">
              {contactInfo.map((contact, index) => (
                <div key={index} className="flex items-start space-x-3 justify-center md:justify-start">
                  {/* Circle icon */}
                  <div className="w-10 h-10 flex items-center justify-center rounded-full bg-white/10 text-success-400 flex-shrink-0">
                    <contact.icon className="text-base" />
                  </div>
                  {/* Text */}
                  <div className="flex-1 text-sm leading-relaxed text-gray-300 text-left md:text-left">
                    {contact.href ? (
                      <a
                        href={contact.href}
        ...
        >
          <p className="text-xl md:text-2xl font-semibold text-success-300 flex items-center justify-center gap-2">
            "Invest Rationally. Live Happily." 
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="bg-black/70 text-gray-300 text-xs leading-relaxed p-6 rounded-lg mt-6 space-y-4 text-center md:text-left"
        >
          <p>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="flex flex-wrap justify-center md:justify-end gap-4 text-xs">
                <Link to="/privacy-policy" className="footer-link hover:text-success-300">Privacy Policy</Link>
                <Link to="/terms-of-service" className="footer-link hover:text-success-300">Terms of Service</Link>
                <Link to="/disclaimer" className="footer-link hover:text-success-300">Disclaimer</Link>
                <Link to="/commission-disclosure" className="footer-link hover:text-success-300">Commission Disclosure</Link>
              </div>
            </div>
          </div>
        </div>
        ...
        <button
          onClick={scrollToTop}
          className="fixed right-6 bottom-6 w-12 h-12 bg-success-500 rounded-full flex items-center justify-center text-white hover:bg-success-600 transition-all duration-300 shadow-lg hover:shadow-xl z-50"
